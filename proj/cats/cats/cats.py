...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    qualify_sentences = [p for p in paragraphs if select(p)]

    if k < len(qualify_sentences):
        return qualify_sentences[k]
    else:
        return ""
...
def about(subject):
    ...
    def contains_subject(paragraph):
        preprocessed_text = split(lower(remove_punctuation(paragraph)))
...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 or len(source_words) == 0:
        return 0.0
    score = sum([1 for i in range(len(typed_words)) if i < len(source_words) and typed_words[i] == source_words[i]])/len(typed_words)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[x[i+1]-x[i] for i in range(len(x)-1)] for x in times_per_player]  # You may remove this line
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_words_lists = [[] for _ in range(len(times))]
    # For each word, find the player who typed it the fastest
    for i, word in enumerate(words):
        # Find the player with the smallest time for this word
        fastest_player = min(range(len(times)), key=lambda p: get_time(times, p, i))
        # Add the word to the fastest player's list
        fastest_words_lists[fastest_player].append(word)
